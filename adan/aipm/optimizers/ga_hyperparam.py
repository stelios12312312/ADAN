...
#                                     error_score=error_score)
            ...
            _score = _fit_and_score(estimator=individual.est, X=X, y=y, scorer=scorer,
                                         train=train_index, test=test_index, verbose=verbose,
                                         parameters=parameters, fit_params=fit_params,
                                         error_score=error_score)
            _score = _score['test_scores']
        ...
        if searchobj.verbose and (searchobj.num_evaluations + searchobj.num_cache_hits) % searchobj.population_size == 0:
            print("Scoring evaluations: {0}, Cache hits: {1}, Total: {2}".format(searchobj.num_evaluations, searchobj.num_cache_hits, searchobj.num_evaluations + searchobj.num_cache_hits))
        if iid:
...
class EvolutionaryAlgorithmSearchCV(BaseSearchCV):
    ...
    @best_score_.setter
    def best_score_(self,value):
        self._best_score_ = value
    ...
    @best_params_.setter
    def best_params_(self,value):
        self._best_params_ = value
    ...
    def _fit(self, X, y, parameter_dict):
        ...
        if self.verbose:
            ...
            print("Scoring evaluations: %d, Cache hits: %d, Total: %d".format(self.num_evaluations, self.num_cache_hits, self.num_evaluations + self.num_cache_hits))
