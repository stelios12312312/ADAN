...
df=readData(os.path.dirname(os.path.abspath(__file__))+"/iris.csv")

df2,target,scaler=prepareData(df,target_name)
g=findFeaturesGP(df=df2,targets=target,ngen=ngen,max_tree=5,score_weight=5,population=400,features=10,evaluator=evalANOVANumba,task="classification")
